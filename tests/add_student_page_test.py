...
from tests.class_page_test import TestClassPage
...
class TestAddStudent(BaseTest):
    ...
    def test_a_create_single_student(self):
        ...
        assert CLASS_NAME in self.classpage.get_class_name_inside_table(), "Name is not the same"
        assert "1" == self.classpage.get_student_count().text(), "Count is other than 1"
